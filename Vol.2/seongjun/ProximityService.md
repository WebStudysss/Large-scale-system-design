...
![img_12.png](img/ch01/img_12.png)
...
- 개략적인 과정은 아래와 같다.
  1. 전 세계를 자오선과 적도 기준 사분면으로 나눈다. <br/>
      ![img_3.png](img/ch01/img_3.png)
     - 위도 범위 [-90, 0]은 0에 대응
     - 위도 범위 [0, 90]은 1에 대응
     - 경도 범위 [-180, 0]은 0에 대응
     - 경도 범위 [0, 180]은 1에 대응 <br/>
  3. 각각의 격자를 또 다시 사분면으로 나눈다. (이때 각 격자는 경도와 위도 비트를 앞서 살펴본 순서대로 반복하여 표현함)
  4. 이 절차를 원하는 정밀도를 얻을 때까지 반복한다. (통상적으로 base32 표현법을 사용)<br/>
      ![img_4.png](img/ch01/img_4.png)
...
- 샤딩(sharding)을 적용하기 좋은 후보다. (테이블 데이터를 한 서버에 담을 수 없을 수도 있음)
  - 사업장 ID를 기준으로 한다.
  - 모든 샤드에 부하를 고르게 분한할 수 있다.
  - 운영적 측면에서 관리하기 쉽다.
...
- 지오해시를 사용한다. (단순함)<br>
  1. 각각의 지오해시에 연결되는 모든 사업장 ID를 JSON 배열로 만들어 같은 열에 저장한다.
    ![img_8.png](img/ch01/img_8.png)
     - 특정한 지오해시에 속한 모든 사업장 ID가 한 열에 보관된다.<br>
  2. 같은 지오해시에 속한 사업장 ID 각각을 별도 열로 저장한다.<br>![img_9.png](img/ch01/img_9.png)
...
🔼 지리 정보 색인 테이블 사례
- 2번째 방안을 추천한다.
  - 지오해시와 사업장 ID 칼럼을 합친 (geohash, business_id)를 복합 키로 사용하면 사업장 정보를 추가하고 삭제하기 쉽다. (락X)
 
- 지리 정보 색인의 규모 확장
  - 읽기 연산의 빈도가 높다면 서버 한 대의 CPU와 네트워크 대역폭으로는 요청 전부를 감당하지 못할 수 있다. (서버 부하 분산이 필요함)
    - ✔️ 읽기 연산을 지원할 사본 데이터베이스 서버를 늘린다.
    - 샤딩을 적용한다. (까다로움)

<br/>

### 캐시
- 벤치마킹과 비용 분석에 주의해야 한다.

<br/>

> **캐시 키**

- 사용자 위치 정보는 이동할 때마다 미세하게 바뀔 수 있으므로 적절하지 않다.
- 지오해시나 쿼드트리는 같은 격자 내 모든 사업장이 같은 해시 값을 갖도록 만들어 해당 문제를 효과적으로 해결할 수 있다.
...
- 격자 내 사업장 ID (사업장 정보는 상대적으로 안정적이어서 자주 변경되지 않음)
  - 주어진 지오해시에 대응되는 사업장 목록은 다음 질의를 통해 구할 수 있다.
    ```sql
    SELECT business_id FROM geohash_index WHERE geohash LIKE `{: geohash}%`
    ```
  - 주어진 지오해시에 대응되는 사업장 목록을 요청 받으면 일단 캐시를 먼저 조회한다. 캐시에 없는 경우에는 위의 질의를 사용하여 사업장 목록을 DB에서 가져와 다음 캐시에 보관한다.
    ```java
...
- 클라이언트 애플리케이션에 표시할 사업장 정보
  - 키는 business_id, 값은 사업장 이름, 주소, 사진 등의 정보를 담은 객체다.
  - 이 데이터를 보관할 레디스 캐시를 최종 설계 도면에서 `사업장 정보`라고 지칭한다.
...
- 근처 사업장 ID부터 전부 확보한 다음 그 사업장 정보를 전부 추출하여 필터링하면 된다.

<br/>

### 최종 설계도

![img_10.png](img/ch01/img_10.png)
...
4. LBS는 인접한 지오해시를 계산한 다음 목록에 추가한다. (list_of_geohashes = [my_geohash, neighbor1_geohash, ..., neighbor8_geohash])
...
- 사업장 상세 정보를 확인하기 위해 사업장 정보 서비스는 우선 해당 데이터가 `사업장 정보` 레디스 서버에 기록되어 있는지 살핀다.
- 캐시되어 있는 경우 해당 데이터를 읽어 클라이언트로 반환하고, 없는 경우 데이터베이스 클러스터에서 정보를 읽어 캐시에 저장 후 반환한다.
- 새로 추가/갱신되는 정보는 다음날 반영된다는 것을 사업장과 합의하였으므로, 캐시에 보관된 정보 갱신은 밤사이 작업을 돌려 처리할 수 있다.


# 4. 마무리
- IT 기업에서 널리 쓰이는 기술은 지오해시, 쿼드트리, 그리고 S2이다.
- 본 설계안에서 지오해시를 사용하여 지리 정보 색인의 동작 원리를 알아보았다.
  - 캐시를 활용한 지연 시간 감소 방법, 캐시 대상 정보, 빠르게 주변 사업장을 검색하기 위한 캐시 활용법 등
  - 복제와 샤딩을 통한 데이터베이스 규모 확장법
  - LBS를 여러 지역과 가용성 구역에 설치하여 가용성을 높이고, 사용자와 서버 사이 통신 지연을 줄이고, 각 지역에 고유한 사생활 보호법을 준수하는 방법


![img_11.png](img/ch01/img_11.png)
